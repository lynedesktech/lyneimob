  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("negocios")
        .select(
          "*, clientes(id, nome, telefone, email), imoveis(id, titulo, codigo_interno, tipo), lotes(id, quadra, numero_lote, unidade, valor, loteamento_id, loteamentos(id, nome)), usuarios(id, nome), pipeline_etapas(id, nome, cor, icone, tipo, ordem)",
          { count: "exact" }
        )
