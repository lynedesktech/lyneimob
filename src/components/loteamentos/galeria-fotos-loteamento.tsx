...
import type { LoteamentoFoto } from "@/types/database"

type GaleriaFotosLoteamentoProps = {
  loteamentoId: string
  fotos: LoteamentoFoto[]
}

export function GaleriaFotosLoteamento({
  loteamentoId,
  fotos: fotosIniciais,
}: GaleriaFotosLoteamentoProps) {
  const [fotos, setFotos] = useState<LoteamentoFoto[]>(fotosIniciais)
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (fotos.length + arquivos.length > 20) {
      toast.error("Limite máximo de 20 fotos por loteamento")
      return
    }
    ...
    for (const arquivo of Array.from(arquivos)) {
      ...
      const caminho = `${organizacao.id}/${loteamentoId}/${nomeArquivo}`

      const { error: erroUpload } = await supabase.storage
        .from("loteamento-fotos")
  ...
  async function excluirFoto(foto: LoteamentoFoto) {
    const urlObj = new URL(foto.url)
    const caminhoStorage = urlObj.pathname.split("/loteamento-fotos/").pop()
        ...
        {fotos.length > 0 ? (
                ...
                .map((foto) => (
                  <CarouselItem key={foto.id} className="basis-1/2 sm:basis-1/3 lg:basis-1/4">
                    <div className="group relative aspect-video overflow-hidden rounded-lg border">
                      <Image
                        src={foto.url}
                        alt={foto.descricao ?? "Foto do loteamento"}
                        fill
          ...
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12 text-center">
            ...
            <p className="mb-4 text-sm text-muted-foreground">
              Adicione fotos para valorizar o loteamento
            </p>
            <Button
              variant="outline"
              onClick={() => inputRef.current?.click()}
